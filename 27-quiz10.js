...
let restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    const freeSeats = restaurants.filter(
      (element) =>
        element.totalSeats - element.numberOfCustomers > numberOfPeople
    );

    return freeSeats.map((element) => element.name);
  },
  findRestaurantServingDish: function (dishName) {
    const foodIsAvailble = restaurants.filter((element) =>
      element.menu.includes(dishName)
    );
    return foodIsAvailble.map((element) => element.name);
  },
  countNumberOfRestaurantsInArea: function (area) {
    const numberOfResturant = restaurants.filter(
      (element) => element.address.area === area
    );
    return numberOfResturant.length;
  },
};
